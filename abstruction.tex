...
\begin{figure}[htbp]
    ...
    \caption{A graph containing a cycle of length 4}
...
A finite graph $G$ is identified with a generalized metric space, which is structurally a $[0, \infty]$-enriched category. We define a map $|\cdot|$ by $|x| = q^x$ for $x \in \ZZ_{\geq 0} \cup \{\infty\} = Z$. Since the distance function $d$ on a graph $G$ takes values in $Z$, this map satisfies the properties required for $G$ being $[0, \infty]$-enriched category. Here, we assume $q^{\infty} = 0$.
