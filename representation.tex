...
% 日本語フォントをゴシック体に設定（スライドで見やすくするため）
\renewcommand{\kanjifamilydefault}{\gtdefault}

% Theme selection: 派手で目立つテーマ
\usetheme{AnnArbor}
\usecolortheme{crane} % 黄・オレンジ系
...
\begin{frame}{背景と動機}
  \begin{itemize}
    \item \textbf{Magnitude} は、豊穣圏（Enriched Category）に対する「サイズ」の不変量である (Leinster)。
    \item グラフは、豊穣圏（一般化距離空間）の一種とみなせる。
    \item \textbf{Magnitude Homology} は、Magnitude の \textit{categorification}（圏化）として導入された (Hepworth \& Willerton)。
  \end{itemize}

  \begin{block}{研究の主目的}
    グラフの Magnitude Homology の性質、特に特定の条件下における \textbf{Mayer-Vietoris 完全列} の成立について研究すること。
  \end{block}
\end{frame}

\section{グラフのMagnitude}

\begin{frame}{定義: グラフのMagnitude}
  $G=(V,E)$ を有限グラフとする。
  $Z_G$ を類似度行列 (similarity matrix) とし、その成分を $(Z_G)_{xy} = q^{d(x,y)}$ で定義する。

  \begin{definition}[Magnitude]
    $Z_G$ が $\mathbb{Q}(q)$ 上で可逆であるとき、グラフ $G$ の magnitude は次のように定義される：
    \[
  ...
  \vspace{0.5em}
  重要な性質: これは「濃度」（オイラー標数）のように振る舞う。
  % \boxprod -> \square, \sqcup -> \sqcup
  \[ \#(G \square H) = \#G \cdot \#H, \quad \#(G \sqcup H) = \#G + \#H \]
\end{frame}
...
\section{豊穣圏 (Enriched Categories)}
