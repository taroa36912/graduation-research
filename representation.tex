...
\begin{frame}{実際の計算例: 完全グラフ $K_n$}
  ...
  \begin{align*}
    \# K_n(q) &= \frac{n}{1 + (n-1)q} = n \sum_{k=0}^{\infty} - \{(n-1)q \}^k
  \end{align*}
  \vspace{-1.0em}
...
\begin{frame}{projectである例 / でない例}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      \graphimg[height=3cm, keepaspectratio]{K_mn_2.pdf}
      projecting
    \end{column}
    ...
    \vrule \hfill
    \begin{column}{0.3\textwidth}
      \graphimg[height=3cm, keepaspectratio]{C_3_red.pdf}
      not projecting
    \end{column}
  \end{columns}
