...
\begin{frame}{グラフのMagnitude}
  ...
  \begin{definition}[Leinster \cite{Leinster2}, 2016]
    グラフ $G$ の \textbf{similarity matrix} $Z_G (q)$を, その成分が $Z_G(x,y) = q^{d(x,y)}$ である $|V(G)| \times |V(G)|$-行列として定義する.
  \end{definition}
  ここで，$q$ は変数であり，$q^{\infty} = 0$ とする.

  このとき，$Z_G(q)$ の対角成分は全て $1$ なので，$\det Z_G$ の定数項は $1$ であり, よって $Z_G$ は可逆である.
  \begin{definition}[Leinster \cite{Leinster2}, 2016]\label{magnitude}
...
\begin{frame}{例: 完全グラフ $K_3$}
  ...
  その逆行列 $Z_{K_3}^{-1}$ は
  \[
    Z_{K_3} = \frac{1}{(1+2q)(q-1)} \begin{pmatrix} -q-1 & q & q \\ q & -q-1 & q \\ q & q & -q-1 \end{pmatrix}.
  \]
...
\begin{frame}{実際に計算する}
  \begin{lemma}[Leinster \cite{Leinster2}, 2016]\label{vertex_transitive_magnitude}
...
\begin{frame}{組み合わせ的表現}
  \begin{proposition}[Leinster \cite{Leinster2}, 2016]
    $G$ をグラフとするとき, 
    \[
    \begin{split}
...
\section{主結果 1}
\begin{frame}{主結果}
  \begin{theorem}[Leinster \cite{Leinster2}, 2016]
...
\begin{frame}{Magnitude complex}
  \begin{definition}[Hepworth, Willerton \cite{HepworthWillerton}, 2019]
    グラフ $G$, 非負整数 $k,l$ に対し, 自由アーベル群
    \[
    \begin{split}
      MC_{k,l}(G) = \Span_{\ZZ} \bigl\{ (x_0, \dots, x_k) \bigm| x_i \in V(G), x_0 \neq \cdots \neq x_k, \\
      d(x_0, x_1) + \cdots + d(x_{k-1}, x_k) = l \bigr\}
    \end{split}
    \]
...
\begin{frame}{Magniude との関係}
  \begin{theorem}[Hepworth, Willerton \cite{HepworthWillerton}, 2019]
    $G$ をグラフとするとき, 
    \[
...
\begin{tikzpicture}[
  ...
  \begin{scope}[shift={(-3, 0)}]
    ...
    \node[dot, draw=blue, fill=blue] (r2) at (2, 0.5) {};
    \node[dot, draw=blue, fill=blue] (r3) at (1.5, -0.5) {};
...
\begin{frame}{Enriched category から見た グラフ の magnitude}
  ...
  \begin{proof}
    グラフの頂点集合 $V(G)$ に，最短経路が定める距離 $d$ を入れることで，$V(G)$ は一般化距離空間となる. これを $G$ の距離とする.
