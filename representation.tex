...
\begin{frame}{グラフのMagnitude}
  ...
  \begin{definition}
    グラフ $G$ の similarity matrix $Z_G (q)$を, その成分が $Z_G(x,y) = q^{d(x,y)}$ である $|V(G)| \times |V(G)|$-行列として定義する.
  \end{definition}
...
\begin{frame}{実際に計算する}
  \begin{lemma}\label{vertex_transitive_magnitude}
...
\begin{frame}{Magnitude Homology の例}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \centering
      \graphimg[width=0.7\linewidth]{C_5.pdf}
    \end{column}
    \begin{column}{0.5\textwidth}
      各 $k, l$ についての $MH_{k,l}(C_5)$ のランクは以下の通り:
      \includegraphics[width=0.9\linewidth]{Tables/out/mag_hom_C_5_v2.pdf}
