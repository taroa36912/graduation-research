...
\begin{frame}{weighting}
  ...
  \]
  これを, weighting equation と呼ぶ. weighting equation を満たす他の関数 $V(G) \to \QQ(q)$ は存在しない.
\end{frame}
...
\begin{frame}{Proposition 1 の証明の概略}
  \begin{proof}
    $\tilde{w}_G:V(G) \rightarrow \ZZ[q]$ を, 定理1行目で $x_0$ をfixしたものとする↓
    \[
      \tilde{w}_G(x) = \sum_{k=0}^{\infty} (-1)^k \sum_{x = x_0 \neq x_1 \neq \cdots \neq x_k} q^{d(x_0, x_1) + d(x_1, x_2) + \dots + d(x_{k-1}, x_k)}.
    \]
    これが weighting equation を満たすことを示せばよい．
  \end{proof}
\end{frame}
...
\begin{frame}{Proposition 1 からわかること}
  ...
  \end{corollary}
  \begin{proof}
    Proposition 1 で, $c_1, c_2$ の定義を確認する.
  \end{proof}
\end{frame}
...
\begin{frame}{convex}
  \begin{definition}
    ...
    $U$ が \textit{convex} in $X$ であるとは, 任意の頂点 $x,y \in V(U)$ に対し, $d_U(x,y) = d_X(x,y)$ が成り立つことをいう.
  \end{definition}
...
\begin{frame}{projection}
  \begin{definition}
    $X$ をグラフ, $U$ を convex in $X$ な $X$ の部分グラフとする. また, 
    $V_U(X) = \{ v \in V(X) \mid d_X(v,u) < \infty \text{ for some } u \in V(U) \}$ とする. 
    
    $X$ \textit{projects to $U$} とは, 任意の $x \in V_U(X)$ に対し, ある $u' \in V(U)$ が存在して, 任意の $u \in V(U)$ について $d_X(x,u) = d_X(x, u') + d_X(u',u)$ が成り立つことをいう. 各 $x \in V_U(X)$ に対し, そのような $u'$ を1つ固定し, これを $\pi(x)$ と表す.
  \end{definition}
  上記の $u'$ は一意に定まる.
  \begin{center}  
    \graphimg[]{K_mn_2.pdf}
...
\begin{frame}{主結果}
  \begin{theorem}
    ...
    $G \cap H$ が convex in $X$ であり, $H$ projects to $G \cap H$ ならば,
    \[
      \# X = \# G + \# H - \#(G \cap H).
    \]
  \end{theorem}
  ...
  \begin{proof}
    $w_X = w_G + w_H - w_{G \cap H}$ を頑張って示す.
  \end{proof}
\end{frame}
...
\begin{frame}{Magnitude Complexes}
  chain complex $MC_{*, *}(G)$ を定義する.
  \begin{definition}
    グラフ $G$, 非負整数 $k,l$ に対し, $MC_{k,l}(G)$ とは, $V(G)$ の長さ $l$ の列 $(x_0, \dots, x_k)$ によってfreelyにgeneratedされる $\ZZ$-加群である. ただし, 列の要素は, 前後で異なるもの $x_i \neq x_{i+1} (i = 0, \dots, k-1)$ とする.
    微分 $\partial : MC_{k,l}(G) \rightarrow MC_{k-1,l}(G)$ を
    ...
    \]
    で定義する.
    ただし, $l(x_0, \dots, x_k) = d(x_0, x_1) + \dots + d(x_{k-1}, x_k)$ を, 列の長さとする.
  \end{definition}
