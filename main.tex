...
\newpage
\begin{abstract}
The concept of magnitude is introduced by Leinster \cite{Leinster2} and it is defined for enriched categories of finite objects, for example, generalized finite metric spaces such as graphs with finitely many vertices and metric defined aaa.
...
it is a cousin to Euler characteristic. 
The magnitude has properties similar to cardinality. For instance, it satisfies the product rule for Cartesian products and the inclusion-exclusion principle for unions (under mild hypotheses). Formally, the magnitude of a graph is both a rational function over $\QQ$ and a power series over $\ZZ$.

Richard and Simon introduced a bigraded homology theory in \cite{HepworthWillerton} for graphs which has the magnitude as its graded Euler characteristic. They showed how properties of magnitude proved by Leinster categorify to properties such as a Künneth Theorem and a Mayer-Vietoris Theorem. 
...
Many of these cardinality-like invariants arise from a single general invariant. This invariant is called magnitude. 

We take one of examples of magnitude, graphs, to explain the idea of magnitude in this thesis in Section 2.
...
Indeed, graphs are special cases of enriched categories, and the magnitude of graphs is a special case of the magnitude of enriched categories. We shall see this in Section 4.

Finite graphs are viewed as generalized finite metric spaces (see \cref{ex_enriched_cat}(ii)) by considering the set of vertices equipped with the shortest path metric. One of their special properties is that the distances take values in integers. As we shall see, this has the consequence that for a graph $G$, the magnitude $\# G$ is a rational function of $q $ over $\QQ$. (It can also be expressed as a power series in $q$ over $\ZZ$.) We write it as $\# G = \# G(q)$ to avoid confusion with the usage of $G$ for the number of vertices of $G$, while still evoking the analogy with cardinality. One of the cardinality-like properties of magnitude is that 
\[
...
As mentioned above, finite graphs are generalized finite metric spaces. Based on these results, we confirm in the final section that the magnitude of graphs coincides with that defined for enriched categories. \\
...
\begin{proposition}
  Let $(\VV, \otimes, I)$ be a monoidal category and let $\calA, \calB$ be $\VV$-categories.
  Assume that $\VV$ is a symmetric monoidal category, that is, for any objects $u, v \in \VV, u \otimes v \approx v \otimes u$.
  Then, $\calA \otimes \calB$ is a $\VV$-category.
\end{proposition}
...
\begin{theorem}
  Let $G,H$ be graphs.
  Then, the catesian product $G \boxprod H$ coincides with the tensor product $G \otimes H$ of the $[0, \infty]$-categories $G,H$.
\end{theorem}
