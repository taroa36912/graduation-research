...
Sets have cardinality, vector spaces have dimension, and topological spaces have Euler characteristic (under some finiteness condition). 
Many of these cardinality-like invariants arise from a single general invariant, which we call magnitude.

We take one of examples of magnitude, graphs, to explain the idea of magnitude in Section 2 of this thesis.
The full definition of magnitude is framed in the very wide generality of enriched categories having finitely many objects, as introduced by Leinster \cite{Leinster2}.
Indeed, each graph is a special case of enriched categories, and the magnitude of graphs is a special case of the magnitude of enriched categories. We shall see this in Section 4.

Given a finite graph $G$, let $X_G$ be the generalized finite metric space consisting of the set of vertices of $G$ equipped with the shortest path metric (see \cref{ex_enriched_cat}(iii)). By abuse of notation, we denote this metric space $X_G$ simply by $G$. The magnitude $\# G$ of a graph $G$ is a rational function of $q$ over $\QQ$. By the identification of graphs and metric spaces above, the distances of graphs take values in integers, which has the consequence that the magnitude of a graph can also be expressed as a power series in $q$ over $\ZZ$. We write it as $\# G = \# G(q)$ to avoid confusion with the usage of $G$ for the number of vertices of $G$, while still evoking the analogy with cardinality. One of the cardinality-like properties of magnitude is that 
\[
  \# (G \boxprod H)=\# G \cdot \# H \quad (\text{\cite{Leinster1} Lemma 3.6}),
\] 
where $\boxprod$ denotes the cartesian product of graphs (defined in \cref{cartesian_product}).
...
\[
  \# (G \cup H) = \# G + \# H - \#(G \cap H) \quad (\text{\cite{Leinster1} Theorem 4.9}).
\]
For this we must impose some hypotheses. Indeed, Leinster \cite{Leinster1} shows that there is no nontrivial graph invariant that is fully cardinality-like in the sense of satisfying both multiplication and inclusion-exclusion formula without restriction (cf.~\cref{no_graph_inv}). However, the hypotheses we impose are mild enough to cover a wide range of examples, including trees, forests, wedge sums, and certain graphs containing a cycle of length at least 4. For example, let $G$ be a graph shown figure \ref{Example_K_3_wedge_K_2}. 
...
Later, Richard and Simon \cite{HepworthWillerton} categorify the magnitude of graphs by defining the magnitude homology of graphs. This is a bigraded homology theory $MH_{*,*}$. It is functorial with respect to maps of graphs that send vertices to vertices and preserve or contract edges, and its graded Euler characteristic recovers the magnitude:
\[
  \sum_{k,l \geq 0} (-1)^k \rank (MH_{k,l}(G)) q^l = \# G ~ \text{in} ~  \ZZ \llbracket q \rrbracket \quad (\text{\cite{HepworthWillerton} Theorem 2.8}).
\]
...
\[
  5 - 10q + 10q^2 - (10 - 10)q^3 + (10 - 30)q^4 - \dots = \frac{5 - 5q}{1 + q - 2q^3} = \# C_5
\]
...
\begin{table}[htbp]
    \centering
    \caption{The ranks of $MC_{k,l}(C_5)$, the magnitude chain groups of the five-cycle graph \cite{HepworthWillerton}.}
    \label{tab:magnitude_ranks}    
    \includegraphics[width=0.9\linewidth]{Tables/out/mag_hom_C_5.pdf}
\end{table}

Actually, Leinster first introduced the magnitude in the context of enriched categories in \cite{Leinster2}. Generalized metric spaces are examples of enriched categories (see \cref{ex_enriched_cat}(iii)) and the magnitude is defined for them if they are finite. \\

As mentioned above, finite graphs are generalized finite metric spaces. I would like to summarize the main results presented in \cite{HepworthWillerton} and \cite{Leinster1}. Based on these results, we confirm in the final section that the magnitude of graphs coincides with that defined for enriched categories. \\

This thesis is laid out as follows. In Section 2, following \cite{Leinster1}, we define the magnitude of a graph, expressing it as both a rational function and a formal power series over $\mathbb{Z}$. We also review basic properties and examples, including a simple formula for the magnitude of vertex-transitive graphs, prove that magnitude has cardinality-like properties, and derive the inclusion-exclusion formula. In Section 3, based on \cite{HepworthWillerton}, we define the magnitude homology of graphs. We show that its graded Euler characteristic recovers the magnitude and that it satisfies categorified versions of the cardinality-like properties, including a Künneth theorem for cartesian products and a Mayer-Vietoris theorem for unions. In Section 4, referring to \cite{Leinster2}, we introduce the definition of the magnitude of enriched categories and discuss how the magnitude of graphs is derived from this general theory.

We denote $\approx$ as isomorphisms of graphs and groups.

\section*{Acknowledgments} I would like to thank my advisor, Associate Professor Yuta Nozaki, for his continuous support and guidance throughout my research. I would also like to express my sincere gratitude to all the members of the Seminar, Hiroto Miyazi, Kazuki Sugiyama, for their valuable advice and constant encouragement.


\newpage
\section{The Magnitude of Graphs}
This section introduces the magnitude of a graph $G$, along with fundamental examples and properties following \cite{Leinster1}.
...
\begin{definition}[Leinster \cite{Leinster1} Definition 2.1]\label{magnitude_graphs}
...
\begin{example}
  Let $G = K_3$ (complete graph with three vertices as Figure \ref{Example_K_3}). \\
...
\begin{definition}[Leinster \cite{Leinster1}]
...
\begin{lemma}[Leinster \cite{Leinster1} Lemma 2.3]\label{unique property on weighting equation}
...
\begin{example}\label{ex_mag_cal}
  \begin{enumerate}[label=(\roman*)]
    \item $G = V_n$ (edgeless graph with $n$ vertices as in Figure \ref{example_no_edge_graph}). \\
    ...
    \item $G = K_n$ (complete graph with $n$ vertices as in Figure \ref{example_complete_graph}). \\
    ...
    \item $G = C_n$ (cycle graph with $n$ vertices as in Figure \ref{example_cycle_graph}). \\
    ...
    \item $G$ is the Petersen graph (Figure \ref{example_petersen_graph}).\\
    ...
    \item $G = K_{m,n}$(complete bipartite graph as in Figure \ref{example_complete_bipartite_graph}). \\
...
\begin{lemma}[Leinster \cite{Leinster1} Lemma 3.5]\label{disjoint_union}
...
\begin{lemma}[Leinster \cite{Leinster1} Lemma 3.6]\label{cartesian_magnitude}
  Let $G$ and $H$ be graphs. Then,
...
\begin{proposition}[Leinster \cite{Leinster1} Proposition 3.9]\label{sum_of_magnitude}
...
\begin{corollary}[Leinster \cite{Leinster1} Corollary 3.10]\label{count_vertices_edges}
  Let $G$ be a graph.
...
\begin{lemma}[Leinster \cite{Leinster1} Lemma 4.1]\label{no_graph_inv}
...
\begin{definition}[Leinster \cite{Leinster1} Definition 4.2]
...
\begin{definition}[Leinster \cite{Leinster1} Definition 4.6]
...
\begin{example}
  Let $X$ be a bipartite graph and let $e$ be an edge of $X$ (see Figure \ref{example_K_mn_2}). Then, $X$ projects to the subgraph consisting of $e$ and its endpoints.
  \figgraph{K_mn_2.pdf}{A bipartite graph $X$ with a highlighted edge}{example_K_mn_2}
\end{example}
...
\begin{example}
  Let  $G$ be a graph and consider the graph $H$ formed by identifying one of the edges of a cycle graph $C_n$ with an edge of $G$ as in Figure \ref{example_connect_C_4}. Now, let $n \geq  4$. 
...
In this section, we define the magnitude homology of a graph $G$. We provide fundamental examples and properties, and state the Mayer-Vietoris sequence for magnitude homology following \cite{HepworthWillerton}.

\subsection{The Definition of The Magnitude Homology of Graphs}
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 2.1]
...
We mean the tuple $(x_0, \dots, \hat{x_i}, \dots, x_k)$ obtained by removing $x_i$ from $(x_0, \dots, x_k)$.
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 2.2, \textit{Magnitude chain complex}]
  Let $G$ be a graph.
...
\begin{lemma}[Hepworth, Willerton \cite{HepworthWillerton} Lemma 2.11]
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 2.4, \textit{magnitude homology}]
  Let $G$ be a graph.
...
\begin{theorem}[Hepworth, Willerton \cite{HepworthWillerton} Theorem 2.8]\label{categorifying}
  Let $G$ be a graph. Then, 
  \[
...
\begin{proposition}[Hepworth, Willerton \cite{HepworthWillerton} Proposition 2.9]
  Let $G$ be a graph. Then,
  \begin{itemize}
...
\begin{proof} We calculate the magnitude homology groups directly.
  ...
  \[
      ...
      \end{cases}
  \]
  and $\partial = 0$ induces the first statement.
  For $l=1$,
  \[
      ...
      \end{cases}
    \]
  and $\partial = 0$ induces the second statement.
\end{proof}
...
\begin{proposition}[Hepworth, Willerton \cite{HepworthWillerton} Proposition 2.10]
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 3.1, \textit{induced chain maps}]
...
\begin{lemma}
  ...
  Then, if $f$ is a graph map, then $d_H(f(x), f(y)) \leq d_G(x, y)$ holds for any $x,y \in V(G)$.
\end{lemma}
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 3.2, \textit{Induced maps in homology}]
...
\begin{proof}
  ...
  For the second statement, we obtain by definition;
  \[
    f_*(x_0, x_1) = \begin{cases}
...
\begin{proposition}[Hepworth, Willerton \cite{HepworthWillerton} Proposition 4.1]\label{magnitude_of_disjoint_union}
  Let $G$ and $H$ be graphs.
  We define the inclusion graph maps $i : G \rightarrow G \sqcup H, j : H \rightarrow G \sqcup H$. Then,
  \[
    i_* \oplus j_* : MH_{*,*}(G) \oplus MH_{*,*}(H) \rightarrow MH_{*,*}(G \sqcup H)
  \]
...
\begin{proof}
  \begin{itemize}
    We show the surjectivity and injectivity.
    \item Surjectivity.
    Let $(x_0, \dots, x_k) \in MH_{k,l}(G \sqcup H)$ be a generator.
    ...
    \end{align*}
    in $MH_{*,*}(G \sqcup H)$. Now, since $\{x_0, \dots, x_{k_1}\} \subset V(G)$ and $\{y_0, \dots, y_{k_2}\} \subset V(H)$, there is no cancellation between the two terms. Contradiction.
  \end{itemize}
\end{proof}
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 5.2, \textit{exterior product}]
...
\begin{example}
  Let $G = C_2 \boxprod C_2 = C_4$ shown in Figure \ref{example_square_graph}. \\
...
\begin{proof}
  ...
  Let $\bf{x} = (x_0, \dots, x_{k_1}), \bf{y} = (y_0, \dots, y_{k_2})$. Here, we should consider the sequence of tensor products of the magnitude chain complexes defined by
  \begin{align*}
  ...
  \end{align*}
  Then, we should show only the first equality and this follows by direct calculation. 
\end{proof}
...
We do not prove this theorem in this thesis.
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 6.3, 6.4]
...
\begin{theorem}[Hepworth, Willerton \cite{HepworthWillerton}, Theorem 6.5]
...
We do not prove these theorems in this thesis.
...
\begin{definition}[Hepworth, Willerton \cite{HepworthWillerton} Definition 7.1]
  A graph $G$ is said to be \textit{diagonal} if $MH_{k,l}(G) = 0$ for $k \neq l$.
\end{definition}

\begin{proposition}[Hepworth, Willerton \cite{HepworthWillerton} Proposition 7.2]
...
In this section, following \cite{Leinster2}, we explain the motivation for studying the magnitude of graphs in a broader context. We employ the notion of enriched categories to define the magnitude.

\subsection{The Magnitude of a Matrix}
\begin{definition}[Leinster \cite{Leinster2}]
...
\begin{lemma}
  Let $\zeta$ be an invertible $I \times I$-matrix over a rig $k$. Then, $\zeta$ has the magnitude.
\end{lemma}

\begin{proof}
  If $\zeta$ is invertible, then $w = \zeta^{-1} u_I$ and $v = u_I \zeta^{-1}$ obviously satisfy the definition of weighting and coweighting respectively. Thus $\zeta$ has the magnitude by \cref{sum_of_w.c.}.
\end{proof}
...
\begin{definition}\label{def_monoidal_cat}
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \figcommu[width=0.6\linewidth]{triangle_1.pdf}
  \end{enumerate}
\end{definition}

We also call $\VV$ a monoidal category, omitting $\otimes$ and $I$.

\begin{example}\label{ex_monoidal_categories}
  \begin{enumerate}[label=(\roman*)]
...
\begin{definition}
  An \textit{enriched category} $\calA = (\calA, m, j)$ in a monoidal category $(\VV, \otimes, I)$ is defined as follows.
  First, $\calA$ consists of the objects and the morphisms, and endowed with the following data:
  \begin{enumerate}[label=(\arabic*)]
    \item For any objects $a,b$ of $\calA$, $\Hom_{\calA}(a,b)$ is an object of $\VV$.
    \item For any objects $a,b,c$ of $\calA$, $m_{abc}$ is a morphism in $\VV$ given by $m_{abc} : \Hom_{\calA}(b,c) \otimes \Hom_{\calA}(a,b) \rightarrow \Hom_{\calA}(a,c)$, which defines the composition of morphisms.
    \item For any object $a$ of $\calA$, $j_a$ is a morphism in $\VV$ given by $j_a : I \rightarrow \Hom_{\calA}(a,a)$, which defines the identity morphism of $a$.
...
\begin{definition}
  Let $\calA, \calA'$ be $\VV$-categories. A functor $F:\calA \rightarrow \calA'$ is called a \textit{$\VV$-functor} if it satisfies the following conditions. We denote $F_{ab} : \mor_{\calA}(a,b) \rightarrow \mor_{\calA'}(F(a),F(b))$ as the morphism.
  \begin{enumerate}[label=(\arabic*)]
    \item The following diagram commutes for any objects $a,b,c$ of $\calA$:
    \figcommu[width=\linewidth]{square_1.pdf}
    \item The following diagram commutes for any object $a$ of $\calA$:
    \figcommu[]{triangle_4.pdf}
  \end{enumerate}
\end{definition}
...
\begin{example}\label{ex_enriched_cat}
  \begin{enumerate}[label=(\roman*)]
    \item Let $\VV = \textbf{Set}$ be a monoidal category described in \cref{ex_monoidal_categories} (i). Then, $(\mathbf{Cat}, \circ, id)$ is a $\VV$-category. Indeed, $\mor(C,D)$ is a set for any categories $C,D$, $m_{CDE} : \mor(D,E) \times \mor(C,D) \rightarrow \mor(C,E) ; (F,G) \mapsto F \circ G$ and $j_C : \{*\} \rightarrow \mor(C,C) ; * \mapsto id_C$ are maps of \textbf{Set}, $F \circ id_C = F = id_D \circ F$ holds for $F : C \rightarrow D$, and $(F \circ G) \circ H = F \circ (G \circ H)$ holds for $F : D \rightarrow E, G : C \rightarrow D, H : B \rightarrow C$.
    \item Let $\VV = \textbf{Vect}_K$ be a monoidal category described in \cref{ex_monoidal_categories} (ii). We say that a category is a \textit{$K$-linear category} if it is enriched over $\VV$. Then, $\VV$ is a $K$-linear category. For any vector spaces $V,W,T,U$, $\mor_{\VV}(V,W) = \hom_K(V,W)$ is a vector space, $m_{VWT} : \hom_K(W,T) \otimes_K \hom_K(V,W) \rightarrow \hom_K(V,T) ; (f,g) \mapsto f \circ g$ is a linear map, and $j_V : K \rightarrow \hom_K(V,V) ; a \mapsto a \cdot id_V$ is also a linear map. $(f \circ g) \circ h = f \circ (g \circ h)$, and $f \circ id_T = f, g = id_T \circ g$ for any linear maps $f : T \rightarrow U, g : W \rightarrow T, h : V \rightarrow W$.
    \item Let $\VV=[0, \infty]$ be a monoidal category described in \cref{ex_monoidal_categories} (iii). We say that a pair of a set $X$ and a map $d : X \times X \rightarrow [0, \infty]$ is a \textit{generalized metric space} if $d$ satisfies that for any elements $a,b,c \in X, d(a,b) + d(b,c) \geq d(a,c)$ and $d(a,a) \geq 0$ hold. Then, a generalized metric space is a $\VV$-category with morphisms $\mor(a,b) = d(a,b)$. Indeed, for any points $x,y,z$ of a generalized metric space $(X,d)$, $\mor(x,y) = d(x,y) \in [0, \infty]$, $m_{xyz} : \mor(y,z) \otimes \mor(x,y) \rightarrow \mor(x,z) ; d(y,z) + d(x,y) \mapsto d(x,z)$ and $j_x : 0 \rightarrow \mor(x,x) ; 0 \mapsto d(x,x)$ are morphisms in $[0, \infty]$ by the definition above, $d(x,y) + 0 \geq d(x,y), 0 + d(y,z) \geq d(y,z)$ holds for any points $x,y$ of $X$, and $d(x,y) + d(y,z) \geq d(x,z)$ holds for any points $x,y,z$ of $X$, which arrows the composition of morphisms.
    \item Let $\VV = \mathbf{2}$ be a monoidal category described in \cref{ex_monoidal_categories} (iv). Then, a preorder is a $\VV$-category. Indeed, for any elements $a,b,c$ of a preorder $P$, $\mor_P(a,b) = t$ if $a \leq b$ and $\mor_P(a,b) = f$ if $a \nleq b$, $m_{abc} : \mor_P(b,c) \otimes \mor_P(a,b) \rightarrow \mor_P(a,c)$ and $j_a : t \rightarrow \mor_P(a,a)$ are morphisms in $\mathbf{2}$, $a \leq b$ implies $a \leq b$ holds for any elements $a,b$ of $P$, and if $a \leq b$ and $b \leq c$, then $a \leq c$ holds for any elements $a,b,c$ of $P$.
  \end{enumerate}
\end{example}

Indeed, each enriched category above corresponds to the definition of each structure.

\begin{definition}
  Let $(\VV, \otimes, I)$ be a monoidal category and let $\calA, \calB$ be $\VV$-categories. Then, the \textit{tensor product} $\calA \otimes \calB$ is a category defined as follows:
  \begin{enumerate}[label=(\arabic*)]
    \item $\ob(\calA \otimes \calB) = \ob(\calA) \times \ob(\calB)$.
    \item For any objects $(a,b), (a',b')$ of $\calA \otimes \calB$, 
    \[
      \Hom_{\calA \otimes \calB}((a,b), (a',b')) = \Hom_{\calA}(a,a') \otimes \Hom_{\calB}(b,b').
    \]
  \end{enumerate}
\end{definition}
...
\begin{remark}
  Let $(\VV, \otimes, I)$ be a monoidal category and let $k$ be a rig. Then, we define $\mid \cdot \mid$ to be a monoid homomorphism:
  \[  | \cdot | : (\ob(\VV)/{\approx}, \otimes, I) \rightarrow (k, \cdot, 1_k).
  \]
  We endow a monoidal category $\VV$ with $| \cdot |$ and also call this four tuple $(\VV, \otimes, I, |\cdot|)$ a monoidal category.
\end{remark}

Here, we can choose various monoid homomorphisms depending on the purpose.

\begin{example}\label{ex_monoidal_cat_2}
  \begin{enumerate}[label=(\roman*)]
    \item Let $\VV = \textbf{FinSet}$ be a monoidal category described in \cref{ex_monoidal_categories} (i) restricted to finite sets and endowed with the cardinality monoid homomorphism $| \cdot | : \ob(\VV)/{\approx} \rightarrow \QQ_{\geq 0} ; X \mapsto \# X$.
    \item Let $\VV = \textbf{FDVect}_K$ be a monoidal category described in \cref{ex_monoidal_categories} (ii) restricted to finite vector spaces over $K$ and endowed with the dimension monoid homomorphism $| \cdot | : \ob(\VV)/{\approx} \rightarrow \ZZ_{\geq 0} ; V \mapsto \dim_K V$.
    \item Let $\VV=[0, \infty]$ be a monoidal category described in \cref{ex_monoidal_categories} (iii) endowed with the monoid homomorphism $| \cdot | : \ob(\VV)/{\approx} \rightarrow \RR_{\geq 0} ; x \mapsto e^{-x}$.
    \item Let $\VV = \mathbf{2}$ be a monoidal category described in \cref{ex_monoidal_categories} (iv) endowed with the monoid homomorphism $| \cdot | : \ob(\VV)/{\approx} \rightarrow \ZZ ; f \mapsto 0, t \mapsto 1$.
  \end{enumerate}
\end{example}

\begin{definition}[Leinster \cite{Leinster2} Definition 1.3.2]
...
\begin{example}
  \begin{enumerate}[label=(\roman*)]
    \item Let $\VV = \textbf{FinSet}$ be a monoidal category described in \cref{ex_monoidal_cat_2} (i) and let $\calA$ be a $\VV$-category (i.e.~a category with finitely many objects and finite hom sets). Then, $|A| \in \QQ$ is defined.
    \item Let $\VV = \textbf{FDVect}_K$ be a monoidal category described in \cref{ex_monoidal_cat_2} (ii) and let $\calA$ be a $\VV$-category (i.e.~a $K$-linear category with finitely many objects and finite-dimensional hom linear spaces). Then, $|A| \in \QQ$ is defined.
    \item Let $\VV=[0, \infty]$ be a monoidal category described in \cref{ex_monoidal_categories} (iii) and let $\calA$ be a $\VV$-category (i.e.~a generalized finite metric space). Then, $|A| \in \RR$ is defined.
    \item Let $\VV = \mathbf{2}$ be a monoidal category described in \cref{ex_monoidal_categories} (iv) and let $\calA$ be a $\VV$-category (i.e.~a finite preorder). Then, $|A| \in \ZZ$ is defined.
  \end{enumerate}
\end{example}


  \subsection{The Relation of The Magnitudes of Graphs and Enriched Categories}
\begin{theorem}[Leinster \cite{Leinster2}, \cite{Leinster1}]
  The magnitude of a graph $G$ defined in \cref{magnitude_graphs} coincides with the Möbius inversion of the $[0, \infty]$-category corresponding to $G$.
\end{theorem}
