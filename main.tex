...
Actually, Leinster first introduced the magnitude in the context of enriched categories in \cite{Leinster2}. Generalized metric spaces are examples of enriched categories and the magnitude is defined for them if they are finite. \\
...
\begin{example}\label{ex_enriched_cat}
  \begin{enumerate}[label=(\roman*)]
    ...
    \item Let $\VV=[0, \infty]$ be a monoidal category described in \cref{ex_monoidal_categories} (iii). Then, a generalized metric space is a $\VV$-category. Indeed, for any points $x,y,z$ of a generalized metric space $M$, $\mor_M(x,y) = d(x,y) \in [0, \infty]$, $m_{xyz} : \mor_M(y,z) + \mor_M(x,y) \rightarrow \mor_M(x,z) ; d(y,z) + d(x,y) \mapsto d(x,z)$ and $j_x : 0 \rightarrow \mor_M(x,x) ; 0 \mapsto d(x,x)$ are morphisms in $[0, \infty]$, $d(x,y) + 0 \geq d(x,y), 0 + d(y,z) \geq d(y,z)$ holds for any points $x,y$ of $M$, and $d(x,y) + d(y,z) \geq d(x,z)$ holds for any points $x,y,z$ of $M$.
