...
\begin{definition}(magnitude chain complex)
  ...
  \[
    MC_{*,*}(G) = \bigoplus_{l=0}MC_{*, l}(G)
  \]
  For non-negative integers $k$ and $l$, $MC_{k,l}(G)$ is freely generated by tuples $(x_0, \dots, x_k)$ of $V(G)$ satisfying $x_0 \neq x_1 \neq \cdots \neq x_k$ and $l(x_0, \dots, x_k) = l$.
  The coefficient ring is $\mathbb{Z}$.
...
\begin{example}
  $(\mathbb{Z}_{\geq 0}, +, \cdot, 0, 1)$ is a rig.
\end{example}
...
\begin{example}
  Let $G$ be a graph.
  Then, $Z_G(q)$ is a $V(G) \times V(G)$-matrix over the rig $\mathbb{Q}[[q]]$ and the weighting on $Z_G(q)$ is the weighting on $G$ defined in Section 2.1.
\end{example}
...
In this document, we only treat the locally small categories, which means that for any objects $a,b$ of a category $\mathcal{C}$, the hom-set $\hom_{\mathcal{C}}(a,b)$ is a set.



\begin{definition}
  A category $\mathcal{C}$ is called a \textit{monoid} if $\mathcal{C}$ has only one object $*$ and $V=Hom_{\mathcal{C}}(*,*)$ is a monoid with the composition of morphisms as the binary operation and the identity morphism $id_*$ as the identity element. We denote the operation of $V$ by $\otimes$.
  ここに可換図式を挿入
\end{definition}

\begin{definition}
  A pair $(\mathcal{V}, \otimes, I)$ is called a \textit{monoidal category} if it satisfies the following conditions:
  \begin{enumerate}
    \item $\mathcal{V}$ is a category.
    \item $\otimes : \mathcal{V} \times \mathcal{V} \rightarrow \mathcal{V}$ is a functor.
    \item $I$ is an object of $\mathcal{V}$.
    \item There exist the natural isomorphism $\alpha : \otimes \circ (\otimes \times id_{\mathcal{V}}) \Rightarrow \otimes \circ (id_{\mathcal{V}} \times \otimes)$ given by $\alpha_{uvw} : (u \otimes v) \otimes w \isom u \otimes (v \otimes w)$
    \item There exist the natural isomorphism $\lambda : I \otimes - \Rightarrow id_{\mathcal{V}}$ given by $\lambda_u : I \otimes u \isom u$
    \item There exist the natural isomorphism $\rho : - \otimes I \Rightarrow id_{\mathcal{V}}$ given by $\rho_u : u \otimes I \isom u$
    \item The following diagram commutes for any objects $u,v,w,x$ of $\mathcal{V}$:
    ここに可換図式を挿入
    \item The following diagrams commute for any objects $u,v$ of $\mathcal{V}$:
    ここに可換図式を挿入
  \end{enumerate}
\end{definition}
...
\begin{definition}
  An \textit{enriched category} $\mathcal{A}$ in a monoidal category $(\mathcal{V}, \otimes, I)$ is defined as follows:

  \begin{enumerate}
    \item For any objects $a,b$ of $\mathcal{A}$, $\hom_{\mathcal{A}}(a,b)$ is an object of $\mathcal{V}$.
    \item For any objects $a,b,c$ of $\mathcal{A}$, there exists a morphism $m_{abc} : \hom_{\mathcal{A}}(b,c) \otimes \hom_{\mathcal{A}}(a,b) \rightarrow \hom_{\mathcal{A}}(a,c)$ in $\mathcal{V}$, which defines the composition of morphisms.
    \item For any object $a$ of $\mathcal{A}$, there exists a morphism $j_a : I \rightarrow \hom_{\mathcal{A}}(a,a)$ in $\mathcal{V}$, which defines the identity morphism of $a$.
    あと３つの可換図式を挿入
  \end{enumerate}

  Then, $\mathcal{A}$ is called a \textit{$\mathcal{V}$-category}.
\end{definition}

\begin{definition}
  Let $\mathcal{A}, \mathcal{A}'$ be $\mathcal{V}$-categories. A functor $F:\mathcal{A} \rightarrow \mathcal{A}'$ is called a \textit{$\mathcal{V}$-functor} if it satisfies the following conditions. We denote $F_{ab} : \mor_{\mathcal{A}}(a,b) \rightarrow \mor_{\mathcal{A}'}(F(a),F(b))$ as the morphism function.
  \begin{enumerate}
    \item The following diagram commutes for any objects $a,b,c$ of $\mathcal{A}$:
    ここに可換図式を挿入
    \item The following diagram commutes for any object $a$ of $\mathcal{A}$:
    ここに可換図式を挿入
  \end{enumerate}
\end{definition}

\begin{remark}
  The family of all $\mathcal{V}$-categories and $\mathcal{V}$-functors form a category, which is denoted by $\mathcal{V}\textit{-Cat}$.
\end{remark}
...
\begin{definition}
  Let $(\mathcal{V}, \otimes, I)$ be a monoidal category and $k$ be a rig.
