...
\captionsetup[figure]{name=Figure} % 図のラベルを Figure に変更
\captionsetup[table]{name=Table}   % 表のラベルを Table に変更
...
\[
  \# (G \cup H) = \# G + \# H - \#(G \cap H).
\]
For this we must impose some hypotheses. Indeed, Leinster \cite{Leinster1} shows that there is no nontrivial graph invariant that is fully cardinality-like in the sense of satisfying both multiplication and inclusion-exclusion formula without restriction (cf.~\cref{no_graph_inv}). However, the hypotheses we impose are mild enough to cover a wide range of examples, including trees, forests, wedge sums, and certain graphs containing a cycle of length at least 4. For example, let $G$ be a graph shown figure 1. 
...
\begin{example}
  Let $G = K_3$ (complete graph with three vertices as Figure 2). \\
...
\begin{example}\label{ex_mag_cal}
  \begin{enumerate}[label=(\roman*)]
    \item $G = V_n$ (edgeless graph with $n$ vertices as in Figure 3). \\
    ...
    \item $G = K_n$ (complete graph with $n$ vertices as in Figure 4). \\
    ...
    \item $G = C_n$ (cycle graph with $n$ vertices as in Figure 5). \\
    ...
    \item $G$ is the Petersen graph (Figure 6).\\
    ...
    \item $G = K_{m,n}$(complete bipartite graph as in Figure 7). \\
...
\begin{example}
  Let  $G$ be a graph and consider the graph $H$ formed by identifying one of the edges of a cycle graph $C_n$ with an edge of $G$ as in Figure 8. Now, let $n \geq  4$. 
...
\begin{example}
  Let $G = C_2 \boxprod C_2 = C_4$ shown in Figure 9. \\
