...
\begin{example}
  \begin{enumerate}[label=(\roman*)]
    \item $G = V_n$ (edgeless graph with $n$ vertices). Then, $\aut(G) = \mathfrak{S}_n$ and $G$ is vertex-transitive. $S = 1$ and we have $\# V_n = n$.
    \item $G = K_n$ (complete graph with $n$ vertices). Then, $\aut(G) = \mathfrak{S}_n$ and $G$ is vertex-transitive. $S = 1 + (n-1)q$ and we have $\# K_n = \frac{n}{1 + (n-1)q}$.
    \item $G = C_n$ (cycle graph with $n$ vertices). Then, $\aut(G) = D_{2n}$ and $G$ is vertex-transitive.
    ...
    \item $G = K_{m,n}$(complete bipartite graph). Then, $\aut(G) = \mathfrak{S}_m \times \mathfrak{S}_n$ if $m \neq n$ and $G$ is not vertex-transitive. You can calculate the magnitude as follows:
  \end{enumerate}
\end{example}
